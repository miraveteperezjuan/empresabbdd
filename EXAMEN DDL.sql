create DATABASE IF NOT EXISTS examenJuanMiravete;
use examenJuanMiravete;

-- set foreign_key_checks=0  Con este comando desactivo las claves ajenas (FK) para todo el servidor. Si no comrpomete la integridad referencial.alter
...
ALTER TABLE `CENTROS`
  ADD CONSTRAINT `Centros_Departamentos_fk1` FOREIGN KEY (`NUMCE`) REFERENCES `DEPARTAMENTOS` (`NUMCE`) ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `EMPLEADOS`
   ADD CONSTRAINT `Empleados_Departamentos_fk1` FOREIGN KEY (`NUMDE`) REFERENCES `DEPARTAMENTOS` (`NUMDE`) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;